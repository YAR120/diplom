...
const initializeDatabase = () => {
    db.serialize(() => {
        ...
        const initialTopics = [
            { articleId: "article_1", title: "Старі вітряки можуть виробляти на 30% більше енергії" },
            { articleId: "article_2", title: "Сонячні панелі на будь-якій поверхні" },
            { articleId: "article_3", title: "Огляд 2E PSPLW300: портативна сонячна панель" },
            { articleId: "article_4", title: "Чи вигідно встановлювати сонячну батарею на балконі" }
        ];
        ...
        initialTopics.forEach(topic => {
            db.run(
                ...
                err => {
                    if (err) {
                        console.error(`Ошибка добавления темы ${topic.title}:`, err.message);
                    }
                }
            );
        });
            ...
            for (const user of initialUsers) {
                try {
                    ...
                    db.run(
                        ...
                        [user.username, user.fullName, user.email, user.phone, hashedPassword, user.role, user.authToken, user.aboutMe],
                        err => {
                            if (err) {
                                console.error(`Ошибка добавления пользователя ${user.email}:`, err.message);
                            } else {
                                console.log(`Добавлен пользователь: ${user.username}, authToken: ${user.authToken}`);
                            }
                        }
                    );
                } catch (err) {
                    console.error(`Ошибка хеширования пароля для ${user.username}:`, err.message);
                }
            }
            console.log("Начальные пользователи добавлены.");
        } else {
            console.log("Таблица users уже содержит пользователей, пропускаем добавление.");
        }
    });
};
...
const insertTestMessages = () => {
    db.get(`SELECT COUNT(*) as count FROM messages`, (err, row) => {
        ...
        if (row.count === 0) {
            ...
            const testMessages = [
                {
                    ...
                    username: "admin",
                    content: "Хтось пробував модернізувати старі вітряки? Які технології дають +30% ефективності?",
                    createdAt: "2025-04-18T09:00:00Z"
                },
                {
                    topicArticleId: "article_1",
                    username: "olena",
                    content: "Читала про нові лопаті з композитних матеріалів. Вони легші і краще ловлять вітер.",
                    createdAt: "2025-04-18T09:15:00Z"
                },
                {
                    topicArticleId: "article_1",
                    username: "ivan",
                    content: "Так, але композитні лопаті коштують дорого. Може, є дешевші рішення?",
                ...
                },
                {
                    topicArticleId: "article_2",
                    username: "olena",
                    content: "Реально, але треба перевірити міцність даху і кут нахилу. У мене стоять, дають 5 кВт.",
                    createdAt: "2025-04-18T11:20:00Z"
                },
                {
                    topicArticleId: "article_2",
                    username: "admin",
                    content: "А як щодо гнучких панелей? Вони підходять для нерівних поверхонь?",
                    createdAt: "2025-04-18T11:40:00Z"
                },
                {
                    topicArticleId: "article_3",
                    username: "ivan",
                    content: "Цікавить PSPLW300 для походів. Як вона в похмуру погоду працює?",
                    createdAt: "2025-04-18T12:15:00Z"
                },
                {
                    topicArticleId: "article_4",
                    username: "olena",
                    content: "Хочу батарею на балкон. Хтось рахував, за скільки окупається?",
                    createdAt: "2025-04-18T13:00:00Z"
                },
                {
                    topicArticleId: "article_4",
                    username: "admin",
                    content: "Залежить від тарифів. У середньому 5-7 років, якщо економить електрику.",
                    createdAt: "2025-04-18T13:20:00Z"
                }
            ];

            testMessages.forEach(message => {
                db.get("SELECT id FROM users WHERE username = ?", [message.username], (err, user) => {
                    if (err || !user) {
                        console.error(`Пользователь ${message.username} не найден`);
                        return;
                    }
                    db.get("SELECT id FROM topics WHERE articleId = ?", [message.topicArticleId], (err, topic) => {
                        if (err || !topic) {
                        ...
                        db.run(
                            ...
                            err => {
                                if (err) {
                                    console.error(`Ошибка добавления сообщения:`, err.message);
                                } else {
            ...
            console.log("Таблица messages уже содержит записи, пропускаем добавление.");
        }
    });
};
    ...
    db.get(`SELECT COUNT(*) as count FROM orders`, (err, row) => {
        ...
        if (row.count === 0) {
            ...
            testOrders.forEach(order => {
                db.get("SELECT id FROM users WHERE fullName = ?", [order.username], (err, user) => {
                    if (err || !user) {
                        console.error(`Пользователь с fullName ${order.username} не найден`);
                        return;
                    }
                    db.run(
                        `INSERT INTO orders (userId, username, phone, technology, date, status) VALUES (?, ?, ?, ?, ?, ?)`,
                        [user.id, order.username, order.phone, order.technology, order.date, order.status],
                        err => {
                            if (err) {
                                console.error(`Ошибка добавления заказа:`, err.message);
                            } else {
                                console.log(`Добавлен заказ для ${order.username} со статусом ${order.status}`);
                            }
                        }
                    );
                });
...
const getTopicByArticleId = (articleId, callback) => {
    db.get("SELECT * FROM topics WHERE articleId = ?", [articleId], (err, row) => {
        if (err) {
            callback(err);
            return;
        }
        ...
        if (userId) {
            db.run(
                ...
                function (err) {
                    ...
                    db.run(
                        ...
                        function (err) {
                            if (err) {
                                console.error("Ошибка добавления в order_history:", err.message);
                            }
            ...
            db.run(
                ...
                function (err) {
                    ...
                    db.get(`SELECT userId FROM orders WHERE id = ?`, [orderId], (err, row) => {
                        if (err || !row) {
                            console.error("Ошибка получения userId для order_history:", err ? err.message : "Заказ не найден");
                            callback(null, { changes: this.changes });
                            return;
                        }
                        db.run(
...
const saveProject = (project, callback) => {
    const { userId, username, phone, project: projectData, technology, createdAt } = project;
    db.run(
